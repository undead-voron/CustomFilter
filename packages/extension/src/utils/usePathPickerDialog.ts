...
export function usePathPickerDialog() {
  ...
  const getOnmouseoverAction = (...args) => {
    console.log('placeholder for now', args)
  }
  ...
  const show = (
    ...
    for (let i = 0, l = paths.length; i < l; i++) {
      ...
      a.addEventListener('click', getOnclickAction(paths[i], target, onSelect), false)
      a.addEventListener('mouseover', getOnmouseoverAction(paths[i], target), false)
      li.appendChild(a)
      pathPickerUL.value.appendChild(li)
    }
